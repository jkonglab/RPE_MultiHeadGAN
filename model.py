...
class Discriminator(nn.Module):
    ...
    def __init__(self, n_f, in_ch=3, device=torch.device("cpu")):
        ...
        self.net = nn.Sequential(
            ConvBlock(in_ch, n_f, 4, stride=2, padding=1, padding_mode="reflect", activation="leaky"),    # 96 -> 48
            ConvBlock(n_f, n_f*2, 4, stride=2, padding=1, padding_mode="reflect", activation="leaky"),    # 48 -> 24
            ConvBlock(n_f*2, n_f*4, 4, stride=2, padding=1, padding_mode="reflect", activation="leaky"),  # 24 -> 12
            ConvBlock(n_f*4, n_f*8, 4, stride=2, padding=1, padding_mode="reflect", activation="leaky"),  # 12 -> 6
            nn.Conv2d(n_f*8, 1, 1)
        )
        self.net.to(device)
