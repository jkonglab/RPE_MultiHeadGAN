...
def pairedTransform(x, y):
    img = np.concatenate((x, y), axis=2)
    tensor = transforms(img)
    # tx, ty = torch.split(tensor, 1)
    tx = torch.narrow(tensor, 0, 0, 3)
    ty = torch.narrow(tensor, 0, 3, 1)
    tx = tv.transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))(tx)
    return tx, ty
...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        image_A = cv2.resize(image_A, None, fx=3, fy=3, interpolation=cv2.INTER_CUBIC)
        image_B = cv2.resize(image_B, None, fx=3, fy=3, interpolation=cv2.INTER_NEAREST)

        item_A, item_B = pairedTransform(image_A[:, :, 0:3], np.expand_dims(image_B[:, :, 0], 2))
